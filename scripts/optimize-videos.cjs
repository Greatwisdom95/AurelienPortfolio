...
const videosToOptimize = [
    {
        ...
        // URL encoded path might be different on filesystem if unchanged, let's use the one that works in shell
        // Based on previous ls, directory is "AR 3D" (with space) or I tried to rename it?
        // Let's check safely. The previous rename failed so it should still be "AR 3D"
        fileSystemInput: 'public/assets/videos/AR 3D/5B2ECB90-3246-4ACA-875E-474480F6AA4F.mp4',
    ...
    {
        fileSystemInput: 'public/assets/videos/FullStackWebsite/jolisite website (1).mp4',
        output: 'public/assets/videos/FullStackWebsite/website-optimized.mp4'
    },
    {
        fileSystemInput: 'public/assets/videos/FeaturedCLIENTImmorose/Logo immorose animation/084FEDFC-0985-4CFC-A52D-104CA7011B9D (1).mp4',
        output: 'public/assets/videos/FeaturedCLIENTImmorose/Logo immorose animation/immorose-optimized.mp4'
    },
    {
        fileSystemInput: 'public/assets/videos/FeaturedCLIENTImmorose/Logo Miroir mall animation/6E81B99B-68FD-4636-8543-679E9172DBF8.mp4',
...
videosToOptimize.forEach(video => {
    ...
    console.log(`Starting processing: ${path.basename(absInput)} -> ${path.basename(absOutput)}`);

    ffmpeg(absInput)
        .outputOptions([
            '-vcodec libx264',
            '-crf 28',         // Higher CRF = higher compression
            '-preset superfast', // Faster compression
            '-an'              // Remove audio
        ])
        .save(absOutput)
        .on('end', () => {
